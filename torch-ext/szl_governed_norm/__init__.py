...
"""szl_governed_norm — governed normalization kernels for the Hugging Face Kernel Hub.

A pure-PyTorch (universal) kernel from SZL Holdings. Provides correctness-
verified RMSNorm and LayerNorm that run on CPU and CUDA and are
...
Load from the Hub:
    ...
    print(gn.__version__)                 # kernel version string
    print(gn.selfcheck())                 # one-shot correctness + receipt check
    x = torch.randn(4, 1024, dtype=torch.float16, device="cuda")
    y = gn.rms_norm(x, eps=1e-6)          # plain, fast path
    y2 = gn.rms_norm(x, eps=1e-6, governed=True)   # records to the default chain
    print(gn.receipt_head())              # hash-chain head over governed calls

    # Per-call chain (no global-state contention across threads/requests):
...
differentiator is verifiable governance, not raw FLOPs.
...
def selfcheck() -> Dict[str, Any]:
    ...
    so downstream code (and SZL's own a11oy / hatun-mcp) can do
    ``get_kernel("SZLHOLDINGS/szl-governed-norm").selfcheck()`` to confirm the
    loaded kernel is the real, working article before trusting it.
